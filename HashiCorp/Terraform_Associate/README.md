...
Note: Only the required fields must be included
...
Otherwise, for each tf file, there will be resources created that might not be our goal.
...
When we used this local_file, where "local" is the provider ([hashicorp/local](https://registry.terraform.io/providers/hashicorp/local/latest)) and "file" is the resource. So, once pressed terraform init, the hashicorp/local file was downloaded to a secret file called (/root/terraform-local-file/.terraform)
...
Once we press terraform init, we can see that it uses the previously installed Hashicorp/local provider and is now installing the Hashicorp/random provider.
...
The random_string will create 6 6-length string that has no uppercase and no special characters. Whereas the AWS resource will create an instance which is of instance_type m5.large and has a tag. This tag will use the randomly generated string we earlier created.
...
Surely, it won't work like this. If we continue, it will create two key_pairs in the us-east-1 region.

So, let's create another provider that has region ca-central-1 mentioned, and the alias is central here.
...
Now, once you press terraform show, you can see that once a resource was created in us-east-1 and another one in ca-central-1
...
This is another example of an AWS instance.
...
If we have used multiple ways to store the variable, it has an order that follows to use the value
...
To do that, we need to set the output "variable_name"  and provide a value and argument there.
...
If in the future you want to print this ami instance value in the output, 
![alt text](image-56.png)

It will pass this message and remind you that you have set sensitive = true for this variable.
...
The output will display the hidden one
...
Now, if we decide that the string should be of length 5, and apply changes, it will create a random string of length 5 and rename the tag for the instance with this string. 

![alt text](image-64.png)

If we just want to create 5 5-length string but don't want to apply this change to the AWS instance that depends on this string, we can use this

![alt text](image-65.png)

In this way, a random variable of length 5 will be generated, but the name tah for the instance won't have any changes to it.
